...
class cnn:
	def __init__(self, learnRate: float, batchSize: int, checkpointPath: str, testStep: int, saveStep: int):
		...
		self.cnnArgs = [
			{
				"conShape": [3, 3, 3, 4], "conStride": [1, 1, 1, 1], "poolingFilterSize": (3, 3), "poolingStride": (2, 2)
			}
			, {
				"conShape": [3, 5, 4, 4], "conStride": [1, 1, 1, 1], "poolingFilterSize": (3, 3), "poolingStride": (2, 2)
			}
			, {
				"conShape": [3, 3, 4, 8], "conStride": [1, 1, 1, 1], "poolingFilterSize": (3, 3), "poolingStride": (2, 2)
			}
			, {
				"conShape": [3, 3, 8, 16], "conStride": [1, 1, 1, 1], "poolingFilterSize": (3, 3), "poolingStride": (2, 2)
			}
			, {
				"conShape": [3, 3, 16, 32], "conStride": [1, 1, 1, 1], "poolingFilterSize": (3, 3),	"poolingStride": (2, 2)
			}
			, {
		...
		with tf.Session() as sess:
			...
			for p in range(10):
				for j, file in enumerate(self.trainFile):
					...
					plt.title('loss step')
					plt.show()
					print("  [TF] this is the %s" % file, "and ac is ", ac, "var is %f" % self.restrain(board))
