...
class cnn:
			...
			for i in range(500):
				...
				for file in self.trainFiles:
					...
					for j in range(10000//self.batchSize):
						...
						self.savecheckpoint()
					accuracyPoint = sess.run(self.accuracy, feed_dict={self.x_test: testX, self.y_test: testY})
					self.accuracyLine.append(accuracyPoint)
					print("[TF] this is the %d epochs, %s" % (i, file), "and accuracy is ", accuracyPoint, "var is %f" % np.var(np.array(self.lossLine)), "loss is %f" % self.lossLine[-1])
					self.lossMean.append(np.array(self.lossLine[-self.epochSize:]).mean())
					self.drawAndpost()
					
					
	def drawAndpost(self):
		xDisc = np.array(range(self.epoch)) * self.epochSize
		xCon = np.array(range(self.epoch * self.epochSize))
		plt.figure(figsize=(10, 10))
		plt.plot(xDisc, self.accuracyLine, 'r')
		plt.plot(xDisc, self.lossMean, 'g')
		plt.plot(xCon, self.lossLine, 'b')
		plt.annotate(self.accuracyLine[-1], xy=(xDisc[-1], self.accuracyLine[-1]), textcoords='offset points', fontsize=16, arrowprops=dict(arrowstyle='->', connectionstyle="arc3,rad=.2"), xytext=(+30, -30)) 
	...
	def network(self, x_input, reuse, isTrain):
		with tf.variable_scope("cnn", reuse=reuse) as vs:
			...
			"""block 1"""
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 3, 8], strides=[1, 1, 1, 1], act=tf.nn.relu, padding="SAME", name="b1c1")
			# net = tl.layers.Conv2dLayer(net, shape=[3, 3, 16, 32], strides=[1, 1, 1, 1], act=tf.nn.relu, padding="SAME", name="b1c2")
			net = tl.layers.MaxPool2d(net, filter_size=(2, 2), strides=(1, 1), padding="SAME", name="b1p")
			net = tl.layers.BatchNormLayer(net, is_train=isTrain, act=tf.nn.relu, name="b1b")
			"""block 2"""
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 8, 16], strides=[1, 1, 1, 1], act=tf.nn.relu, padding="SAME", name="b2c1")
			# net = tl.layers.Conv2dLayer(net, shape=[3, 3, 48, 64], strides=[1, 1, 1, 1], act=tf.nn.relu, name="b2c2")
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 16, 32], strides=[1, 2, 2, 1], act=tf.nn.relu, padding="SAME", name="b2c3")
			net = tl.layers.MaxPool2d(net, filter_size=(2, 2), strides=(1, 1), padding="SAME", name="b2p")
			net = tl.layers.BatchNormLayer(net, is_train=isTrain, act=tf.nn.relu, name="b2b")

			"""block 3"""
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 32, 48], strides=[1, 1, 1, 1], act=tf.nn.relu, padding="SAME", name="b3c1")
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 48, 64], strides=[1, 1, 1, 1], act=tf.nn.relu, name="b3c2")
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 64, 128], strides=[1, 2, 2, 1], act=tf.nn.relu, padding="SAME", name="b3c3")
			net = tl.layers.MaxPool2d(net, filter_size=(2, 2), strides=(1, 1), padding="SAME", name="b3p")
			...
			net = tl.layers.DenseLayer(net, act=tf.nn.relu, n_units=512, name="Dense1")
			net = tl.layers.DropoutLayer(net, keep=0.9, is_train=isTrain, is_fix=True, name="dropout1")
			net = tl.layers.DenseLayer(net, act=tf.nn.relu, n_units=16, name="Dense2")
			net = tl.layers.DropoutLayer(net, keep=0.9, is_train=isTrain, is_fix=True, name="dropout2")
			net = tl.layers.DenseLayer(net, act=tf.nn.relu ,n_units=128, name="Dense3")
