...
class cnn:
	...
	def network(self, x_input, reuse, isTrain):
		with tf.variable_scope("cnn", reuse=reuse) as vs:
			...
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 8, 16], strides=[1, 1, 1, 1], act=tf.nn.relu, padding="SAME", name="b1c2")
			...
			"""block 2"""
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 16, 32], strides=[1, 1, 1, 1], act=tf.nn.relu, padding="SAME", name="b2c1")
			# net = tl.layers.Conv2dLayer(net, shape=[3, 3, 48, 64], strides=[1, 1, 1, 1], act=tf.nn.relu, name="b2c2")
			net = tl.layers.Conv2dLayer(net, shape=[3, 3, 32, 32], strides=[1, 2, 2, 1], act=tf.nn.relu, padding="SAME", name="b2c3")
